/**
 * Created by GAELMP on 03/04/2018.
 */
 angular.module("etudiant.auth",[]);
angular.module("etudiant.etude",[]);
var app=angular.module("myapp",["etudiant.auth","etudiant.etude","ngRoute"]);
...
function EtudiantController($http){
    ...
    this.getitem=function(inde,appren){
        this.etudiant=angular.copy(appren);
    ...
    function update(etudiant)
    {
        ...
        vm.etudiant=angular.copy(etudiant);
    }
    ...
    vm.getfaculte();
    this.modifieretud=function(etudiant){
        $http.put('/etudiant/'+ etudiant.id, etudiant)
            .then(function(response){
                console.log('la reponse est ', response.data);
                vm.get();
            })
